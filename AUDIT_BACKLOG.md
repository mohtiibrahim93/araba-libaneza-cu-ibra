...
| **No refund workflow** *(confirmed still open)* | Repo-wide search for "refund" returns zero matches. | High | Missing Feature | M | Open |
...
| **Kids-deposit checkout has no recoverable error path** *(confirmed still open, not re-verified live — network block)* | `RegistrationFormSection.tsx` kids+`payDeposit` branch clears draft/marks complete before `create-checkout` resolves. | Medium | Bug | S | Open |
...
| **Name field has no format validation** *(confirmed still open)* | Emoji-only name accepted with no error; DB migration only bounds length, not format. | Low | Bug / Data Quality | S | Open |
| **`.env` is committed to git** *(confirmed still open, long-standing)* | `git ls-files` includes `.env`; no `.gitignore` entry. Anon/publishable keys only — anti-pattern, not a live leak. | Low today | Security / Tech Debt | XS | Open |
