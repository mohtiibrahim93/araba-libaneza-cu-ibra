(() => {
 ...
 function saveEdit(id,ar,ro,variants){
  ...
  }catch(e){toast(String((e&&e.message)||'Nu s-a putut salva.'));}
 }
 function editForm(c){
 ...
 document.addEventListener('submit',ev=>{if(ev.target.id==='write-form'){ev.preventDefault();const val=$('#typed-answer').value.trim();if(val)answer(val);else toast('Scrie un răspuns înainte de verificare.');}if(ev.target.dataset&&ev.target.dataset.editForm){ev.preventDefault();const f=new FormData(ev.target);
   ...
   if(ev.target.id==='fix-form'){ev.preventDefault();const val=$('#fix-answer').value.trim();if(val)tryFix(val);else toast('Scrie răspunsul corect ca să exersezi.');}});
 let searchTimer;
