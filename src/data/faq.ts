/**
 * Every FAQ the site answers — one source for the six on the homepage and the
 * full list at /intrebari-frecvente (EN: /en/faq).
 *
 * The homepage used to render all thirty-nine in a single stacked accordion,
 * which is why the section ran so long. It now shows the six marked `featured`
 * in a 3x2 grid and links here for the rest. Only what is visible on a page is
 * marked up as FAQPage on that page — Google requires the structured data to
 * match the rendered content, so the homepage emits six and the full page emits
 * all of them.
 *
 * To promote a question to the homepage, set `featured: true` on it; a test
 * asserts each language marks exactly SHORT_FAQ_COUNT of them.
 */
export type QA = { q: string; a: string; featured?: boolean };
export type FAQGroup = { title: string; items: QA[] };

/** How many questions the homepage strip shows — two rows of three. */
export const SHORT_FAQ_COUNT = 6;

export const FAQ_CONTENT: Record<"ro" | "en", { groups: FAQGroup[] }> = {
  ro: {
    groups: [
      {
...
/** The questions shown on the homepage, in author order. */
export const featuredFaqs = (lang: "ro" | "en"): QA[] =>
  FAQ_CONTENT[lang].groups.flatMap((g) => g.items).filter((i) => i.featured);

/** Every question, flattened — used for the full page's FAQPage markup. */
export const allFaqs = (lang: "ro" | "en"): QA[] =>
  FAQ_CONTENT[lang].groups.flatMap((g) => g.items);

/** FAQPage structured data for whichever subset a page actually renders. */
