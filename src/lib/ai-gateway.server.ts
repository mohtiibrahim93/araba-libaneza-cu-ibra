// Lovable AI Gateway plumbing. Server-only: LOVABLE_API_KEY never reaches the
// browser, so nothing here may be imported from a component.
//
// The gateway mints one X-Lovable-AIG-Run-ID per request and we have to resend
// it on follow-up calls inside the same server request, otherwise the run id is
// lost from the usage logs. That is all the fetch wrapper below does.
